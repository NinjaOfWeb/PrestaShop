...
class BlockCategories extends Module
{
	...
	public function hookLeftColumn($params)
	{
		...
		$groups = $id_customer ? implode(', ', Customer::getGroupsStatic($id_customer)) : (int)Configuration::get('PS_UNIDENTIFIED_GROUP');
	...
	public function hookFooter($params)
	{
		$id_customer = (int)($params['cookie']->id_customer);
		// Get all groups for this customer and concatenate them as a string: "1,2,3..."
		$groups = $id_customer ? implode(', ', Customer::getGroupsStatic($id_customer)) : (int)Configuration::get('PS_CUSTOMER_GROUP');
		$id_product = (int)(Tools::getValue('id_product', 0));
		$id_category = (int)(Tools::getValue('id_category', 0));
		$id_lang = (int)($params['cookie']->id_lang);
		$smartyCacheId = 'blockcategories|'.$this->context->shop->id.'_'.$groups.'_'.$id_lang.'_'.$id_product.'_'.$id_category;
